...
import {
  ...
  Logger,
  Query,
  DefaultValuePipe, // Query, DefaultValuePipe
  NotFoundException,
  Post,
  Body,
  HttpCode,
...
export class AdminChoicesController {
  ...
  async findAll(
    ...
    this.logger.log(
      `Request received for finding all choices ${sourceNodeId ? `for sourceNodeId: ${sourceNodeId}` : ''}`,
    );
  ...
  async create(
    ...
    try {
      ...
      throw new InternalServerErrorException(
        'An unexpected error occurred during choice creation.',
      );
    }
  }
  ...
  async update(
    ...
    try {
      const updatedChoice = await this.adminChoicesService.update(
        id,
        updateChoiceDto,
      );
      ...
      this.logger.error(
        `Unhandled error during choice update for ID ${id}: ${error}`,
      );
      throw new InternalServerErrorException(
        'An unexpected error occurred during choice update.',
      );
    }
  }
  ...
  async remove(@Param('id', ParseIntPipe) id: number): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        `Unhandled error during choice removal for ID ${id}: ${error}`,
      );
      throw new InternalServerErrorException(
        'An unexpected error occurred during choice removal.',
      );
    }
  }
}
