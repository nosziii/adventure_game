...
import { Injectable, Inject, NotFoundException, Logger, InternalServerErrorException, ConflictException } from '@nestjs/common';
...
export class AdminNodesService {
    ...
    private dtoToDbNode(dto: CreateNodeDto | UpdateNodeDto): Partial<StoryNode> {
        ...
        if (dto.health_effect !== undefined) dbData.health_effect = dto.health_effect;
        if (dto.item_reward_id !== undefined) dbData.item_reward_id = dto.item_reward_id;
    ...
    async findOne(id: number): Promise<StoryNode> {
         ...
         const node = await this.knex<StoryNode>('story_nodes').where({ id }).first();
         if (!node) {
    ...
    async remove(id: number): Promise<void> {
         ...
         try {
             ...
             if (error.code === '23503') { // PostgreSQL foreign key violation error code
                 throw new ConflictException(`Cannot delete node ${id} because other records (e.g., choices) depend on it.`);
             }
