...
const mapChoiceRecordToChoiceAdminDto = (
  choice: ChoiceRecord,
): ChoiceAdminDto => {
  return {
    id: choice.id,
...
export class AdminNodesController {
  ...
  async create(
    @Body(/* new ValidationPipe() - ha nincs globálisan */)
  ...
  async update(
    ...
    try {
      const updatedNode = await this.adminNodesService.update(
        id,
        updateNodeDto,
      );
  ...
  // --- Graph Végpont ---
  @Get('graph/data') // Útvonal: /api/admin/nodes/graph/data
  @Roles('admin')
  async getStoryGraph(): Promise<StoryGraphDto> {
    ...
    return {
      ...
      choices: choices.map(mapChoiceRecordToChoiceAdminDto), // Használjuk a choice mappert is
    };
  }
}
