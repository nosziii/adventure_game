import { IsString, IsNotEmpty, IsIn, IsInt, ValidateIf } from 'class-validator';
...
export class CombatActionDto {
  ...
  @ValidateIf((o: CombatActionDto) => o.action === 'use_item')
  @IsNotEmpty({ message: 'itemId is required when action is use_item' })
  @IsInt({ message: 'itemId must be an integer' })
  itemId?: number;

  @ValidateIf((o: CombatActionDto) => o.action === 'use_ability') // Csak akkor validáljuk, ha az akció 'use_ability'
  @IsNotEmpty({
