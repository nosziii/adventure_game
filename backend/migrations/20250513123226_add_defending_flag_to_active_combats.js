// backend/migrations/YYYYMMDDHHMMSS_add_defending_flag_to_active_combats.js
const TABLE_NAME = 'active_combats';
const COLUMN_NAME = 'character_is_defending';
    ...
    await knex.raw(`
      ...
      BEGIN
        IF NOT EXISTS (SELECT FROM information_schema.columns WHERE table_schema = 'public' AND table_name = '<span class="math-inline">\{TABLE\_NAME\}' AND column\_name \= '</span>{COLUMN_NAME}') THEN
          ALTER TABLE public.${TABLE_NAME} ADD COLUMN ${COLUMN_NAME} BOOLEAN NOT NULL DEFAULT FALSE;
      ...
      END $$;
    `);
    console.log(`Raw SQL migration for ${COLUMN_NAME} column in ${TABLE_NAME} finished.`);
  } catch (error) {
...
exports.down = async function(knex) {
  ...
  try {
    ...
    console.log(`Finished removing ${COLUMN_NAME} column from ${TABLE_NAME}.`);
  } catch (error) {
